        ...
        <div class="layui-row">
            ...
            <div class="content">
                <div class="layui-row">
                    ...
                    <div class="layui-btn-group float-l">
                        <a href="<?= Url::to(['article/index','type'=>'draft-box'])?>" class="layui-btn layui-btn-normal">草稿箱</a>
                        <a href="<?= Url::to(['article/index','type'=>'recycle-box'])?>" class="layui-btn layui-btn-danger">回收站</a>
                        <a href="<?= Url::to(['article/index','type'=>'index'])?>" class="layui-btn layui-btn-warm">全部文章</a>
                    </div>
                    <div class="float-r">
                ...
                <table class="layui-table" lay-data="{url:'<?= Url::to(['get-table-data','type'=>$type]);?>', page:true, id:'idTest'}" lay-filter="table-data">
                    <thead>
                    <tr>
    ...
    layui.use(['table','layer', 'jquery'], function(){
        ...
        table.on('tool(table-data)', function(obj){
            ...
            if(obj.event === 'del'){
                ...
                //放入回收站
                layer.confirm('确定恢复到文章列表吗？', function(index){
                    layer.close(index);
                    $.ajax({
                        url : UrlManager.createUrl('/admin/content/article/restore',{id:data.id}),
                        type : 'post',
                        data : {'id' : data.id, '_csrf' : "$token"},
                        success : function(d){
                           if(d.errno === 0){
        ...
        var active = {
            getCheckData: function(){ //获取选中数据
                ...
                $.ajax({
                    url : UrlManager.createUrl('/admin/content/article/batch-del'),
        ...
        $('.batch-del').on('click', function(){
            layer.confirm('您要将这些文章放入回收站吗？', function(index){
                active.getCheckData();
            });
