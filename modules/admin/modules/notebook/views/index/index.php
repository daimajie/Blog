            ...
            <div class="content">
                <div class="layui-row">
                    <div class="layui-btn-group float-l">
                        <a href="javascript:;" class="layui-btn cate_add">添加日记</a>
                        <a href="javascript:history.go(-1);" class="layui-btn">返回</a>
                        <a href="javascript:;" class="layui-btn batch-del">批量删除</a>
                    </div>
                    <div class="float-r">
                        <div class="layui-input-inline">
                            <input type="text" name="name"  placeholder="请输入创建时间" class="layui-input">
                        </div>
                ...
                <table class="layui-table" lay-data="{url:'<?= Url::to(['get-table-data']);?>', page:true, id:'idTest'}" lay-filter="table-data">
                    ...
                    <tr>
                        ...
                        <th lay-data="{field:'username', width:180}">署名</th>
    ...
    layui.use(['table','layer', 'jquery'], function(){
        ...
        table.on('tool(table-data)', function(obj){
            ...
            if(obj.event === 'del'){
                layer.confirm('真的删除行么', function(index){
                    layer.close(index);
                    $.ajax({
                        url : UrlManager.createUrl('/admin/notebook/index/delete',{id:data.id}),
                        type : 'post',
                ...
                layer.open({
                  ...
                  shade: 0.8,
                  area: ['80%', '580px'],
                  content: UrlManager.createUrl('/admin/notebook/index/update', {id:data.id}),
                  end: function(){ //此处用于演示
        ...
        var active = {
            getCheckData: function(){ //获取选中数据
                ...
                $.ajax({
                    url : UrlManager.createUrl('/admin/notebook/index/batch-del'),
        ...
        $('.cate_add').on('click', function(){
            layer.open({
              ...
              shade: 0.8,
              area: ['80%', '580px'],
              content: UrlManager.createUrl('/admin/notebook/index/create'),
              end: function(){ //此处用于演示
                window.location.reload();
              }
            }); 
